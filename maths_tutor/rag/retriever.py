...
from maths_tutor.config import TOP_K_CHUNKS, MIN_SIMILARITY_SCORE
from maths_tutor.embeddings.embedder import Embedder, get_embedder
from maths_tutor.embeddings.vector_store import VectorStore, SearchResult
...
class RetrievalResult:
    ...
    def format_for_prompt(self) -> str:
        ...
        formatted_parts = []
        for i, (chunk, source) in enumerate(zip(self.chunks, self.sources), 1):
            source_info = f"[Source: {source.get('source_file', 'unknown')}, Page {source.get('page_number', '?')}]"
            formatted_parts.append(f"--- Excerpt {i} {source_info} ---\n{chunk}")
...
class Retriever:
    ...
    def search_multiple(
        ...
        for query in queries:
            ...
            for chunk, score, source in zip(result.chunks, result.scores, result.sources):
        ...
        combined = list(zip(all_chunks, all_scores, all_sources))
        combined.sort(key=lambda x: x[1], reverse=True)
        combined = combined[:self.top_k]
        
        if combined:
            chunks, scores, sources = zip(*combined)
            return RetrievalResult(
                chunks=list(chunks),
                scores=list(scores),
...
if __name__ == "__main__":
    ...
    console.print(Panel.fit(
        "[bold blue]RETRIEVER TEST[/bold blue]",
        border_style="blue"
    ))
    ...
    for query in test_queries:
        ...
        if result.has_results:
            for i, (chunk, score, source) in enumerate(zip(result.chunks, result.scores, result.sources), 1):
                console.print(f"\n[bold]Result {i}[/bold] (score: {score:.4f})")
                console.print(f"Source: {source.get('source_file', '?')}, Page {source.get('page_number', '?')}")
